...
public abstract class SocketClient{
	...
	public SocketClient(String name, String address, int connectWaitTime, int readerIdleTimeSeconds, int writerIdleTimeSeconds,
		...
		this.address = new InetSocketAddress(addrs[0], Integer.valueOf(addrs[1]));
		this.name = name;
		...
		synchronized (channelId) {
			...
			if (isWaitSendOk) {
				...
				if (result0.isSuccess()) {
					if (logger.isDebugEnabled()) {
						logger.debug("socketclient[" + name + "] ,channelId[" + channelId + "] send message ok");
					}
					return true;
				} else {
					logger.error("socketclient[" + name + "] ,channelId[" + channelId + "] send message error", result0.cause());
					return false;
				}
			} else {
				return true;
